...
class _EsriMapState extends State<EsriMap> with AutomaticKeepAliveClientMixin {
  ...
  SearchCategory? _activeCategory;
  ...
  // ---------------------------------------------------------------------------
  // POI class fetch (delegates to service)
  // ---------------------------------------------------------------------------

  Future<void> _loadPoiClasses() async {
    final classes = await EsriMapService.fetchAllPoiClasses();
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      final results = await Future.wait([
        EsriMapService.queryBuildings(query),
        EsriMapService.queryPOIs(query),
      ]);
  ...
  Future<void> _performCategorySearch(SearchCategory category) async {
    setState(() {
      _isSearching = true;
      _showResults = false;
      _showSuggestions = false;
    ...
    try {
      final allResults =
          await EsriMapService.queryPOIsByClass(category.poiClassValue);

      // Always plot ALL pins
  ...
  // ---------------------------------------------------------------------------
  // Routing (delegates to service, handles graphics/viewpoint locally)
  // ---------------------------------------------------------------------------
...
/*   Future<void> _launchDirections(MapSearchResult result) async {                                                                
  ...
  Future<void> _solveRoute(MapSearchResult destination, {String? travelMode, (double, double)? originLatLng}) async {
    ...
    try {
      final routeData = await EsriMapService.solveRoute(
      ...
      _routeTravelTimeMinutes = routeData.totalTimeMinutes;
      _routeManeuvers = routeData.maneuvers;
      ...
      final extent = routeData.routeGeometry.extent;
  ...
  String _labelForClass(String classValue) {
    final match = searchCategories.where(
        (c) => c.poiClassValue.toLowerCase() == classValue.toLowerCase());
  ...
  Widget _buildCategoryChip(BuildContext context, SearchCategory category) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
